...
public class SiblingFinder {
    ...
    public static int findLargestSibling(int firstSibling) {
        ...
        findSiblings("", digits, siblingsSet);
    ...
    private static void findSiblings(String prefix, String remaining, Set<String> siblingsSet) {
        if (remaining.isEmpty()) {
            siblingsSet.add(prefix);
        } else {
            for (int i = 0; i < remaining.length(); i++) {
                findSiblings(prefix + remaining.charAt(i),
